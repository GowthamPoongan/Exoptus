...
class AuthService {
  ...
  async googleSignIn(
    ...
    // Handle new nested response structure: { success: true, data: { token, user, redirectTo } }
  ...
  async checkAuth(): Promise<{ isAuthenticated: boolean; user?: User }> {
    ...
    if (response.success && response.data?.valid) {
      await this.cacheUser(response.data.user);
      return {
    ...
    if (refreshed) {
      const retryResponse = await api.get<SessionResponse>("/auth/session");
      if (retryResponse.success && retryResponse.data?.valid) {
        await this.cacheUser(retryResponse.data.user);
        return {
          isAuthenticated: true,
